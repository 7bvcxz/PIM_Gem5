...
from m5.objects import PIMsim, AddrRange, Port, MemCtrl
...
# >> KKM << 22/10/19 Newly added this file
def config_pim(mem_type: str, num_chnls: int) -> Tuple[str, str]:
    """
    ...
    pimsim_mem_configs = os.path.join(pimsim_dir, "configs")

    input_file = os.path.join(pimsim_mem_configs, mem_type + ".ini")

    # Run checks to ensure the `ext/PIMsim` directory is present, contains
    # the configs directory, and the configuration file we require.
    if not os.path.isdir(pimsim_dir):
        raise Exception(
            "The `ext/PIMsim` directory cannot be found.\n"
            "Please navigate to `ext` and run:\n"
            "git clone git@github.com:umd-memsys/DRAMsim3.git"
        )
    elif os.path.isdir(pimsim_mem_configs):
        raise Exception(
            "The `ext/PIMsim/configs` directory cannot be found."
        )
...
class PIMSimMemCtrl(PIMsim):
    ...
    def __init__(self, mem_name: str, num_chnls: int) -> None:
        ...
        """
        super().__init__()
        ini_path, outdir = config_pim(mem_name, num_chnls)
...
class SingleChannel(AbstractMemorySystem):
    ...
    def __init__(self, mem_type: str, size: Optional[str]):
        ...
        """
        super().__init__()
        self.mem_ctrl = PIMSimMemCtrl(mem_type, 1)
        self._size = toMemorySize(size)
        if not size:
            raise NotImplementedError(
                "PIMsim memory controller requires a size parameter."
            )
